...
do
  ...
  echo ${job_name} >> tmp.$$
  qsub -l h_vmem=6G -S /bin/sh -cwd -V -N $job_name -q $servers -o $logs/$job_name.log -e $logs/$job_name.error ${APP_PATH}/scripts/submitConf.sh $currDir $rnd 
  sleep 10
done 
...
qsub -l h_vmem=10G -S /bin/sh -V -cwd -N ${params}.collect -hold_jid `cat tmp.$$ | tr '\n' ','` -q $servers -V -o $logs/collect.log -e $logs/collect.error $APP_PATH/scripts/submitCollectCA.sh ${currDir}
