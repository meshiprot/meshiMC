classdef ConfigEnergyMaxScoreWeightedErrorsAndMedianCorrelation_Gdt_Cts < ConfigEnergy
    properties
    end
...
%            energy = 0.10*sqrt(mean(errors.*errors))+0.9*(1-median(correlations));
        ...
        function [error, correlation] = evaluateTarget(obj,configuration,scoreFunction,coefs,targetObjective,coefOptimization,iTarget)
            ...
            model_neighbor_corr = [];
            for j = 1:targetObjective.nModels
                model_neighbors = find(targetObjective.comparisonTables{2}.CMTable(j,:)>0.95)';
                model_neighbor_corr(j) = 0;
                if (length(model_neighbors)>1)
                    model_neighbor_corr(j) = corr(predictions(model_neighbors), targetObjective.values(model_neighbors));
                end
            end
