classdef ConfigEnergyMWSumErAndMCorrCMTWeightsGDT_TS_NModels < ConfigEnergy
    properties
    end
...
%            energy = 0.10*sqrt(mean(errors.*errors))+0.9*(1-median(correlations));
        ...
        function [error, correlation] = evaluateTarget(obj,configuration,scoreFunction,coefs,targetObjective,coefOptimization,iTarget)
            ...
            weight = wScore + max(wScore,wPredScore);       
            weight = weight.*targetObjective.comparisonTables{2}.weights * length(targetObjective.comparisonTables{2}.weights);
