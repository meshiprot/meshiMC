classdef ConfigEnergyErrorCorrelationEnrichment < ConfigEnergy
    properties
    end
...
%            energy = 0.10*sqrt(mean(errors.*errors))+0.9*(1-median(correlations));
            energy = 0.99*sqrt(mean(errors.*errors))+0.01*(1-median(correlations)) + 1/(1+sqrt(mean(enrichments.*enrichments)));
        end
