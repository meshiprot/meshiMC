classdef ConfigEnergyMWSumErAndMCorrCMTWeightsRMS_NModels < ConfigEnergy
    properties
    end
...
%            energy = 0.10*sqrt(mean(errors.*errors))+0.9*(1-median(correlations));
        ...
        function [error, correlation] = evaluateTarget(obj,configuration,scoreFunction,coefs,targetObjective,coefOptimization,iTarget)
            ...
            weight = weight.*targetObjective.comparisonTables{1}.weights * length(targetObjective.comparisonTables{1}.weights);
