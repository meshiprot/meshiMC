classdef ConfigEnergyMaxScoreWeightedErrorsAndMedianCorrelation_Rms_Cts < ConfigEnergy
    properties
    end
...
%            energy = 0.10*sqrt(mean(errors.*errors))+0.9*(1-median(correlations));
        ...
        function [error, correlation] = evaluateTarget(obj,configuration,scoreFunction,coefs,targetObjective,coefOptimization,iTarget)
            ...
            model_neighbor_corr = nan(targetObjective.nModels,1)';
            for iModel = 1:targetObjective.nModels
                model_neighbors = targetObjective.comparisonTables{1}.neighbors{iModel};
                if (length(model_neighbors)>1)
                    model_neighbor_corr(iModel) = corr(predictions(model_neighbors), targetObjective.values(model_neighbors));
                end
            end
            if (~isempty(model_neighbor_corr(~isnan(model_neighbor_corr))))
                correlation = mean(model_neighbor_corr(~isnan(model_neighbor_corr)));
            end
            
            %error       = ( weight/sum(weight) ) * sqrt(mean(diff.*diff));
            if (nargin > 5)
                    %%%%%%%% for dubugging %%%%%%%%%
